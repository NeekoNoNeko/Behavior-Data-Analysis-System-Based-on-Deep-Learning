...
CLS_MODEL  = r'C:\workspace\Behavior-Data-Analysis-System-Based-on-Deep-Learning\ResNet\best_model.pth'  # ResNet 分类权重
# =======================================
...
CLS_NAMES = ['0other', '1sit', '2resting_chin', '3hunchback']  # 根据你的类别修改

# ---------- 1. 加载模型 ----------
pose_model = YOLO(POSE_MODEL, task='pose')

def load_resnet_model(model_path):
    model = models.resnet50(num_classes=len(CLS_NAMES))  # 如果你的模型结构不同请修改
    model.load_state_dict(torch.load(model_path, map_location='cpu'))
    model.eval()
    return model

resnet_model = load_resnet_model(CLS_MODEL)
...
def classify_skeleton(gray_img):
    ...
    return CLS_NAMES[predicted.item()]
...
def inference_one_frame(frame_bgr):
    ...
    # 分类
    label = classify_skeleton(skel_gray)
